...
### Greenbone (OpenVAS / GSA, opcional)

El stack [Greenbone Community Edition](https://greenbone.github.io/docs/latest/22.4/container/index.html) se activa con el perfil Compose **`greenbone`** (muchas imágenes, RAM y disco en la primera sincronización de feeds).

**Sin OpenVAS** (solo lo que el compose define sin ese perfil, p. ej. MySQL y app):
...
Si no puedes hacer `pull` del registro pero ya tienes las imágenes en local: añade `--pull never` al comando anterior.

Guía detallada (UI, variables, red del escáner, registro, feeds, mitigaciones de informes): [`docs/security/greenbone-openvas.md`](docs/security/greenbone-openvas.md).
